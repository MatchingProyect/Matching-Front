...
export default function Historial(){
    ...
    useEffect(()=>{
        const fetchTeams = async()=>{
            try {
                const {data} = await axios(`/teamMatchByUser/${userLogeado.id}`)
                if(data.status) setTeams(data.teamMatchByUser)
            } catch (error) {
                throw error.message
            }
        }
    ...
    useEffect(()=>{
        const fetchHistorial = async() =>{
            try {
                const dataHistorial = teams.map(async(team) => {
                    const {data} = await axios(`/resultadoMarcador/${team.TeamMatchId}`)
                    if(data.status) return data.resultado
                })
